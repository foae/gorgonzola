...
lint:
	...
	which golangci-lint; if [ $$? -ne 0 ]; then curl -sfL https://raw.githubusercontent.com/golangci/golangci-lint/master/install.sh | sudo sh -s -- -b $(GOPATH)/bin/golangci-lint v1.21.0 && chmod +x $(GOPATH)/bin/golangci-lint; fi
